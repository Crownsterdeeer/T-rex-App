...
function setup() {
  ...
  gameOver = createSprite(width-300,height-250);
  gameOver.addImage(gameOverImg);
  
  restart = createSprite(width-300,height-200);
...
function draw() {
  ...
  if(gameState === PLAY){
    ...
    if(touches.length > 0 || keyDown("space")&& trex.y >= height-82) {
        trex.velocityY = -12;
        jumpSound.play();
        touches = [];
    }
...
function spawnClouds() {
 ...
 if (frameCount % 60 === 0) {
    ...
    cloud.y = Math.round(random(height-200,height-600));
